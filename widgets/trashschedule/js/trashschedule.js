...
vis.binds['trashschedule'] = {
    ...
    getShiftedColor: function (hex, lightnessShift) {
        ...
        return (
            this.toPaddedHexString(rgb[0], 2) + this.toPaddedHexString(rgb[1], 2) + this.toPaddedHexString(rgb[2], 2)
        );
    },
        ...
        return svg.replace(/%23([a-f\d]{6})/gi, function (x) {
            switch (x) {
                case '%238a8a8a':
                    ...
                    break;
                case '%23363636':
                    return `%23${vis.binds['trashschedule'].getShiftedColor(rgb, -0.33)}`;
                    break;
                case '%23595959':
                    return `%23${vis.binds['trashschedule'].getShiftedColor(rgb, -0.19)}`;
                    break;
                case '%23666666':
                    return `%23${vis.binds['trashschedule'].getShiftedColor(rgb, -0.14)}`;
                    break;
                case '%236e6e6e':
                    return `%23${vis.binds['trashschedule'].getShiftedColor(rgb, -0.11)}`;
                    break;
                case '%23707070':
                    return `%23${vis.binds['trashschedule'].getShiftedColor(rgb, -0.1)}`;
                    break;
                case '%239e9e9e':
                    return `%23${vis.binds['trashschedule'].getShiftedColor(rgb, 0.08)}`;
                    break;
                case '%23a3a3a3':
                    return `%23${vis.binds['trashschedule'].getShiftedColor(rgb, 0.1)}`;
                    break;
                ...
                if (!trashType._completed) {
                    if (limit === 0 || rendered < limit) {
                        ...
                        $('<div class="dumpster"></div>')
                            .html(trashType.daysLeft)
                            .wrapInner('<span class="daysleft"></span>')
                            .appendTo(newItem);

                        if (showDate) {
                            $('<span class="nextdate"></span>')
                                .html(new Date(trashType.nextDate).toLocaleDateString(dateLocale, dateOptions))
                                .appendTo(newItem);
                        }
                        ...
                        if (trashType._color) {
                            newItem
                                ...
                                .css(
                                    'background-image',
                                    vis.binds['trashschedule'].getBackgroundImage(trashType._color),
                                );
                        }
